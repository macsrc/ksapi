...
class KSTradeApi():
    def __init__(self, access_token=None, userid=None, consumer_key=None, ip=None, app_id="", host=None, proxy_url=None, \
        ...
        for host in hosts:
            ...
            configuration = ks_api_client.Configuration(host=host, userid=userid, access_token=access_token,
                                                        ...
                                                        consumer_secret=consumer_secret, ip=ip, app_id=app_id,
            ...
            if configuration.proxy_url:
                ...
                if configuration.proxy_user:
                    configuration.proxy_headers = make_headers(
                        proxy_basic_auth=':'.join((configuration.proxy_user, configuration.proxy_pass)))
    ...
    def place_order(self, order_type, instrument_token, transaction_type, \
        ...
        if order_type == "O":
            order = NewOrder(instrumentToken=instrument_token, tag=tag, transactionType=transaction_type, \
                             ...
                             validity=validity, triggerPrice=trigger_price, product=product,
                             smartOrderRouting=smart_order_routing)
            ...
            order = NewSOROrder(instrumentToken=instrument_token, tag=tag, transactionType=transaction_type, \
                                ...
                                disclosedQuantity=disclosed_quantity, triggerPrice=trigger_price)
    ...
    def positions(self, position_type):
        ...
        if position_type == 'TODAYS':
            positions_res = ks_api_client.PositionsApi(self.api_client).positions_today(self.consumer_key,
                                                                                        self.session_token)
        elif position_type == 'OPEN':
            positions_res = ks_api_client.PositionsApi(self.api_client).positions_open(self.consumer_key,
                                                                                       self.session_token)
        elif position_type == 'STOCKS':
            positions_res = ks_api_client.PositionsApi(self.api_client).positions_stocks(self.consumer_key,
                                                                                         self.session_token)
        else:
    ...
    def trade_report(self, order_id=None, is_fno=""):
        ...
        if order_id is None:
            ...
            is_fno = is_fno.capitalize()
            trade_report = ks_api_client.ReportsApi(self.api_client).get_fno_trade_report_by_order_id(order_id,
                                                                                                      self.consumer_key, \
                                                                                                      self.session_token,
                                                                                                      is_fno)
        else:
            trade_report = ks_api_client.ReportsApi(self.api_client).get_trade_report_by_order_id(order_id,
                                                                                                  self.consumer_key, \
                                                                                                  self.session_token)
        return trade_report
    ...
    def margin_required(self, transaction_type, order_info):
        ...
        margin_required = ks_api_client.MarginApi(self.api_client).margin_required(self.consumer_key,
                                                                                   self.session_token,
    ...
    def subscribe(self, input_tokens, callback, broadcast_host=broadcast_host):
        try:
            if self.consumer_secret == None or not self.consumer_secret:
                raise ApiValueError(
                    "Please provide the consumer_secret paramater while creating KSTradeApi object or supply in settings file.")
            proxy = ""
    ...
    def token(self, grant_type, username, password, refresh_token):
        ...
        token = ks_api_client.TokenApi(self.api_client).token_post(authorization=authorization, grant_type=grant_type,
                                                                   username=username, \
    ...
    def revoke(self, token, token_type_hint):
        ...
        AUTH_BASE64 = base64.b64encode(auth_token.encode("UTF-8"))
        authorization = AUTH_BASE64.decode("UTF-8")
        revoke = ks_api_client.TokenApi(self.api_client).revoke_post(authorization=authorization, token=token,
